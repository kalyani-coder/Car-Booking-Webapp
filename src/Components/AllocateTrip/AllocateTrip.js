...
function AllocateTrip() {
  ...
  const fetchTripDetails = async (customerId) => {
    try {
      const response = await fetch(`https://carbookingbackend.onrender.com/api/trip-details/${customerId}`);
      if (!response.ok) {
  ...
  const handleAllocateTrip = () => {
    ...
    axios.post('https://carbookingbackend.onrender.com/api/trip-details', payload)
      .then(response => {
        // Handle success
