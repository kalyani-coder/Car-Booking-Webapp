...
const ViewCustomerRate = () => {
  ...
  useEffect(() => {
    const fetchCustomerRates = async () => {
      try {
        const response = await fetch('https://carbookingbackend.onrender.com/api/customer-rate');
        if (!response.ok) {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`https://carbookingbackend.onrender.com/api/customer-rate/${editedCustomerRate._id}`, {
        method: 'PATCH',
        headers: {
  ...
  const deleteCustomerRate = async (customerRateId) => {
    ...
    try {
      const response = await fetch(`https://carbookingbackend.onrender.com/api/customer-rate/${customerRateId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        throw new Error('Network response was not ok');
