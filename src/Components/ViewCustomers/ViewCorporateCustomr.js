...
const CorporateCustomers = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8787/api/corporate-customer')
  ...
  const handleDelete = (customerId) => {
    ...
    if (confirmed) {
      try {
        ...
        fetch(`http://localhost:8787/api/corporate-customer/${customerId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (!response.ok) {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await fetch(`http://localhost:8787/api/corporate-customer/${editedCustomer._id}`, {
        method: 'PATCH',
        headers: {
