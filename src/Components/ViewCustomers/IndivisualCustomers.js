...
const IndivisualCustomers = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:7000/api/indivisual-customer')
  ...
  const handleDelete = (customerId) => {
    ...
    if (confirmed) {
      try {
        ...
        fetch(`http://localhost:7000/api/indivisual-customer/${customerId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (!response.ok) {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await fetch(`http://localhost:7000/api/indivisual-customer/${editedCustomer._id}`, {
        method: 'PATCH',
        headers: {
