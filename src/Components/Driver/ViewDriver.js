...
const ViewDriver = () => {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(`http://localhost:8787/api/add-Drivers/${editedDriver._id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleDeleteDriver = async (driverId) => {
    ...
    if (confirmed) {
    try {
      const response = await fetch(`http://localhost:8787/api/add-Drivers/${driverId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
