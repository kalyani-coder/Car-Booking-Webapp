...
router.patch('/:id' , async(req, res) => {
    ...
    try{
        const UpdatedtripDetails = await newMasterRateSchema.findByIdAndUpdate(tripDetailsId , req.body ,{
            new : true
        })
...
router.delete('/:id' , async(req, res) => {
    ...
    try{
        const deletedCustomeEnquiry = await newMasterRateSchema.findByIdAndRemove(tripDetailsId)
