...
router.patch('/:id', async (req, res) => {
    ...
    try {
        const UpdatedAddVenders = await newRateSchemaVender.findByIdAndUpdate(AddVendersId, req.body, {
            new: true
        })
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const deletedCustomeEnquiry = await newRateSchemaVender.findByIdAndDelete(AddVendersId)
...
router.get("/venderid/:vendorId", async (req, res) => {
    try {
        const data = await newRateSchemaVender.find({vender_id : req.params.vendorId})
        if(data === 0){
